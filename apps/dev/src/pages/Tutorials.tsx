...
import type { TutorialDifficulty, TutorialField } from "../content/tutorials.types";
...
export function Tutorials() {
  useRouteMeta();
  const c = useCopy(TUTORIALS_COPY);
    ...
    <section className="py-16">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="space-y-24">
          {tutorials.map((tut, i) => (
            // Anchors come from the English title so /pt and /es links share them.
            <div key={tut.slug} id={(TUTORIALS.en[i]?.title ?? tut.title).toLowerCase().replace(/\s+/g, '-')}>
              {/* Tutorial header */}
              ...
              <div className="mt-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div className="flex flex-wrap gap-2">
                  {tut.concepts.map((concept) => (
                    <span key={concept} className="inline-flex rounded-full bg-slate-100 px-2.5 py-0.5 text-xs font-medium text-slate-600">{concept}</span>
                  ))}
                </div>
                <Link
                  to={tut.slug}
